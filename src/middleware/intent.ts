...
export function checkSafety(): any {
    return async (req: InferenceRequest, res: Response, next: NextFunction) => {
        const messages: ChatCompletionMessageParam[] = JSON.parse(JSON.stringify(req.f_messages)) || req.f_messages || [];

        messages.push({ role: 'user', content: req.f_message });

        if (req.f_message.trim().length === 0) {
